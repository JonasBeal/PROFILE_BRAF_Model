...
list.of.packages <- c("readxl","knitr","corrplot","ggfortify","reshape2","gridExtra","magrittr", "tictoc", "ggrepel", "broom", "nlme", "lme4", "lmerTest", "eulerr", "plotly", "paletteer", "ggpubr", "cowplot", "ggcorrplot", "tidyverse", "latex2exp", "randomForestSRC")
...
select(rf_performances, -Variable, -Importance) %>% distinct %>%
...
* Original and processed data result in very similar performances for Drugs sensitivity prediction. Nevertheless, for CRISPR sensitivity prediction, performances increased when we use only model genes and even more when we use direcly model profiles. One hypothesis: since we have less samples in CRISPR data, reducing the number of variables (~3500 for original data, ~90 for original data with model genes only, ~40 variables for model profiles) may have a positive influence. This effect is not present in Drugs but we have 2 times more samples (~160 samples for drugs and ~70 for CRISPR). But all in all, we can say that our data processing before model personalization does not seem to significantly decrease the amount of information in the data.
...
filter(rf_performances, Screening=="Drug", Importance>0) %>%
  mutate(Processing=case_when(
         Processing=="original" ~ "Original data\n(cancer and model genes)",
         Processing=="original_red" ~ "Original data\n(model genes only)",
         Processing=="processed" ~ "Processed data\n(model profiles)"
  ...
  guides(fill=FALSE) +
  labs(title = "RF interpretation - Drugs",
       x = "Variable",
...
filter(rf_performances, Screening=="CRISPR", Importance>0) %>%
  ...
  theme(axis.text.x = element_text(size=5, angle = 90)) +
  guides(fill=FALSE) +
  labs(title = "RF interpretation - CRISPR",
       x = "Variable",
       y = "Variable importance")
