...
CL_mut_profile <- read_csv(paste0("Results/Profiles/",model,"_CLext_mut.csv")) %>% rename(Patient_ID=X1) %>%
  select(-matches("_b2")) %>%
...
CL_RNA_profile <- read_csv(paste0("Results/Profiles/",model,"_CLext_RNA_norm.csv")) %>% rename(Patient_ID=X1) %>%
  select(-matches("_b2")) %>%
  rename_all(function(x) gsub("_b1", "", x)) %>%
  select(Patient_ID,
         #SOX10, HGF, EGF, FGF, 
...
import_ALL <- function(sim_case){
  simulations <- read_table2(paste0("Results/Simulations/",model,"_CLext_",sim_case,".txt")) %>%
