...
list.of.packages <- c("readxl","knitr","corrplot","ggfortify","reshape2","gridExtra","magrittr", "tictoc", "ggrepel", "broom", "nlme", "lme4", "lmerTest", "eulerr", "plotly", "paletteer", "ggpubr", "cowplot", "ggcorrplot", "ggh4x", "gghalves", "tidyverse", "UpSetR", "latex2exp", "tictoc", "patchwork", "ggstance")
...
![Graphical abstract of the complete study](Pictures/GA.png)
...
CL_Drugs <- read_xlsx("Data/CL/GDSC2_fitted_dose_response_15Oct19.xlsx") %>%
  ...
  left_join(rename(CL_clin, Patient_ID=model_name) %>% select(Patient_ID, TCGA_label),
            by="Patient_ID") %>%
  filter(Drug_Name %in% c(DC$Drug_Name, "Trametinib")) %>%
...
CL_Metrics <- bind_rows(mutate(CL_Drugs, Screening="Drug"),
  ...
  mutate(Target=if_else(Target=="MEK_b1", "MEK", Target))
...
CL_Metrics_Reduced <- filter(CL_Metrics,
                             Target %in% c("BRAF", "p53", "PI3K", "MEK")) %>%
...
corr <- select(CL_Metrics_Reduced, Patient_ID, Label, Value) %>%
  ...
  summarise(Pearson=cor(Value, Value_y,
                         method="pearson", use="pairwise.complete.obs"),
            Pearson_t=cor.test(Value, Value_y,
...
It is also possible to check whether the sensitivity of cell lines to BRAF inhibition depends on the BRAF status.

```{r paper_plot, eval=TRUE, echo=FALSE, fig.width=12, message=FALSE, warning=FALSE}
plot_data1 <- left_join(CL_mut_profile, select(CL_clin, model_name, TCGA_label),
                       by=c("Patient_ID"="model_name")) %>%
  mutate("BRAF:1"=if_else(is.na(BRAF), 0, 1),
         "RAS:1"=if_else(is.na(RAS), 0, 1),
         "PI3K:1"=if_else(is.na(PI3K), 0, 1),
         "p53:0"=if_else(is.na(p53), 1, 0)) %>%
  select(TCGA_label, contains(":")) %>%
  as.data.frame()

plot_A <- upset(plot_data1,
...
t <- gather(plot_data1, key="Gene", value = "Status", -TCGA_label) %>%
...
plot_data2 <- CL_Metrics %>%
  left_join(CL_mut_profile,
            by="Patient_ID") %>%
  filter((Drug_Name=="PLX-4720" & Metric=="AUC") | (Metric=="CC2" & Target=="BRAF")) %>%
  mutate(Variant=if_else(Metric=="AUC","Drug (PLX-4720)", "CRISPR (Broad)"),
         BRAF=if_else(is.na(BRAF), "Not mutated", "BRAF mutated\n(gain of function)"))

label_data <- group_by(plot_data2, TCGA_label, BRAF, Variant) %>%
  filter(!is.na(Value)) %>%
  summarise(Number=n()) %>%
  ungroup

plot_Ba <- ggplot(filter(plot_data2, Variant=="Drug (PLX-4720)")) +
  ...
  geom_dotplot(aes(x=TCGA_label, y=Value, fill=TCGA_label),
               ...
               show.legend = FALSE) +
  geom_text(data = filter(label_data, Variant=="Drug (PLX-4720)"),
...
plot_Bb <- ggplot(filter(plot_data2, Variant=="CRISPR (Broad)")) +
  geom_boxplot(aes(x=TCGA_label, y=Value, fill=TCGA_label),
  ...
  geom_dotplot(aes(x=TCGA_label, y=Value, fill=TCGA_label),
               position = position_nudge(x = +0.2),
               binaxis='y',dotsize = 1, stackdir = "center") +
  geom_text(data = filter(label_data, Variant=="CRISPR (Broad)"),
            aes(x=TCGA_label, label=paste0("n=", Number)),
            y=-2.8) +
  scale_fill_manual(values = colors_types#,
...
p_prelim <- ggarrange(plot_Abis, patchworkGrob(plot_B))

p_prelim

#ggsave(file="p_prelim.svg", plot=p_prelim, width=15, height=5)
```

# Data reprocessing

Now we re-process the data to ease following analyses. Since the model contains a read-out node named __Proliferation__ we will use it as a proxy to validate with experimental sensitivities.

We also define a normalised variable based on __Proliferation__ level without any drug inhibition (*i.e* $Proliferation_{normalised} = Proliferation_{withDrug} / Proliferation_{withoutDrug}$)
...
## Correaltions for other targets
...
```{r scatter_pub, echo=FALSE, message=FALSE, warning=FALSE, eval=TRUE, fig.height=6, fig.width=12}
...
plot_data_tab <- filter(data_all_clin,
                    ...
                    SimCase == "mut_RNA",
                    !is.na(Value)) %>%
    left_join(select(CL_mut_profile, Patient_ID, BRAF, p53, RAS, PI3K, MEK),
              by = "Patient_ID")
  ...
  genes_mut <- c("BRAF", "RAS", "p53", "PI3K", "MEK")
  genes_RNA <- c("CRAF", "ERK", "PDPK1", "p70", "PI3K")
...
p_table_red <- select(plot_data, SimCase, P_Mutants, P_Rates) %>%
  ...
  p_add <- (p_table_red + p_crispr_p53 + p_crispr_PI3K) +
...
```{r scatter_interactive_PI3K, echo=FALSE, message=FALSE, warning=FALSE, eval=TRUE, fig.height=6, fig.width=9}
...
```{r, multi_omics_PI3K, echo=FALSE, warning=FALSE, fig.width = 10, fig.height=4.5, eval=TRUE}
...
The previous examples showed that it was possible to use the method to study targets other than BRAF. It is of course also possible to study other types of cancer, as illustrated on the related file "Analysis_withThyroid" which adds thyroid cancer cell lines to melanomas and colorectal cancers. 

Finally, it is perfectly possible to use other models, which were not specifically designed for this study. In order to illustrate this technical capacity, a brief analysis is proposed below with the logical model provided by Flobak et al. in their [article](https://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1004426) about gastric cancer.


```{r, additional_Flobak, echo=FALSE, warning=FALSE, eval=TRUE}

import_Flobak <- function(sim_case){
  simulations <- read_table2(paste0("Results/Simulations/Flobak_CL_",sim_case,".txt")) %>%
    select(-Time, -n_profile) %>%
    mutate(Prosurvival=(Prosurvival_b1 + Prosurvival_b2 + Prosurvival_b3)/3,
           Antisurvival=(Antisurvival_b1 + Antisurvival_b2 + Antisurvival_b3)/3,
           Proliferation=(Prosurvival-Antisurvival+1)/2)
  return(simulations)
}

simulations_Flobak <- sapply(c("mut", "RNA", "mut_RNA"), import_Flobak, simplify = F, USE.NAMES = T) %>% melt %>% 
  filter(variable=="Proliferation") %>%
  spread(key="variable", value="value") %>%
  rename(SimCase=L1) 

data_all_Flobak_z <- filter(simulations_Flobak, Drug_ID=="Zero") %>%
  select(-Drug_ID) %>%
  rename_at(vars(-Patient_ID, -SimCase), function(x) paste0(x, "_noDrug"))
simulations_Flobak %<>%
  rename_at(vars(-Patient_ID, -Drug_ID, -SimCase), function(x) paste0(x, "_Drug")) %>%
  left_join(data_all_Flobak_z, by = c("Patient_ID", "SimCase")) %>%
  #mutate(Proliferation_Norm=Proliferation_Drug-Proliferation_noDrug) %>%
  mutate(Proliferation_Norm=Proliferation_Drug/Proliferation_noDrug) %>%
  filter(Drug_ID %in% c("BRAF", "MEK", "PI3K"))

#Merge with clinical data
simulations_Flobak_clin <- simulations_Flobak %>%
  rename(Target=Drug_ID) %>%
  left_join(mutate(CL_Metrics, Target=if_else(Target=="Raf", "BRAF", Target)),
            by=c("Patient_ID", "Target")) %>%
  left_join(select(CL_mut_profile, Patient_ID, Label_Mut)) %>%
  left_join(select(CL_RNA_profile, Patient_ID, Label_RNA)) %>%
  pivot_longer(cols=starts_with("Proliferation_"),
...
plot_data <- mutate(simulations_Flobak_clin, Output=as.character(Output)) %>% 
  filter(SimCase %in% c("mut",
                        "mut_RNA"),
                    Output=="Prolif. norm.", 
                    Metric == "AUC") %>%
  filter(Drug_Name %in% c("Trametinib", "PLX-4720", "AZD8186" #,"Pictilisib","Alpelisib", "Buparlisib", , "GNE-317"
                          )) %>%
  mutate(Label=paste0(Drug_Name, "\n(", Target, " inhibition)"),
         SimCase=if_else(SimCase=="mut", "Mutations", "Mutations & RNA")) 

ggplot(plot_data, aes(x=Score, y=Value)) +
  geom_point(aes(color=TCGA_label)) +
  facet_grid(Label~SimCase) +
...
It can then be seen that the circular nature of the regulations in this model (no input nodes) makes its behaviour different, with in particular a low impact of mutations. 

We also observe that with a model where the description of BRAF is simpler (a suel Raf node for BRAF and CRAF), the prediction of BRAF inhibition is no longer assured. On the other hand, the better definition of the MEK node, which is more regulated, makes it possible to focus on this inhibition. This inhibition is moderately well predicted, but it is observed in more detail that it is actually well predicted for colorectal cancer only. The proximity between colorectal and gastric cancers (the initial focus of this model) could explain this difference.

```{r, additional_Flobak2, echo=FALSE, warning=FALSE, eval=TRUE}

plot_data <- mutate(simulations_Flobak_clin, Output=as.character(Output)) %>% 
  filter(SimCase %in% c("mut_RNA"),
                    Output=="Prolif. norm.", 
                    Metric == "AUC") %>%
  filter(Drug_Name %in% c("Trametinib"), TCGA_label=="CRC") %>%
  mutate(Label=paste0(Drug_Name, "\n(", Target, " inhibition)"),
         SimCase=if_else(SimCase=="mut", "Mutations", "Mutations & RNA")) 
...
ggplot(plot_data, aes(x=Score, y=Value)) +
  ...
  labs(color="Cancer types:",
       title="With CRC cell lines only")
```