...
criteria_CL %>% filter(Category=="Bimodal", gene_symbol %in% model_genes_all) %>%
  ...
  ggtitle("3 random Bimodal genes") +
  theme_pubclean()
...
criteria_CL %>% filter(Category=="ZeroInf", gene_symbol %in% model_genes_all) %>%
  ...
  geom_histogram(bins = 30) +
  facet_wrap(~key, scales = 'free') +
  ggtitle("3 random Zero-Inflated genes") +
  theme_pubclean()
...
criteria_CL %>% filter(Category=="Unimodal", gene_symbol %in% model_genes_all) %>%
  ...
  geom_histogram(bins = 30) +
  facet_wrap(~key, scales = 'free') +
  ggtitle("3 random Unimodal genes") +
  theme_pubclean()
...
for (model in models){
  write.csv(inverse_mapping(CLmodel_mut_prof,
            ...
            paste0("Results/Profiles/withThyroid/",model,"_CL_mut.csv"))
  
  write.csv(inverse_mapping(CLmodel_CNA_prof,
                            datatype = 'bin',
                            geneindex_var = node_genes_correspondance[[model]]),
            paste0("Results/Profiles/withThyroid/",model,"_CL_CNA.csv"))

  write.csv(inverse_mapping(CLmodel_mutCNA_prof,
                            datatype = 'bin',
                            geneindex_var = node_genes_correspondance[[model]]),
            paste0("Results/Profiles/withThyroid/",model,"_CL_mutCNA.csv"))
  
  write.csv(inverse_mapping(CLmodel_mutCNAneg_prof,
                            datatype = 'bin',
                            geneindex_var = node_genes_correspondance[[model]]),
            paste0("Results/Profiles/withThyroid/",model,"_CL_mutCNAneg.csv"))
  
  write.csv(inverse_mapping(CLmodel_RNA_prof,
                            datatype = 'bin',
                            geneindex_var = node_genes_correspondance[[model]]),
            paste0("Results/Profiles/withThyroid/",model,"_CL_RNA.csv"))
  
  write.csv(inverse_mapping(CLmodel_RNA_prof_norm,
                            datatype = 'norm',
                            geneindex_var = node_genes_correspondance[[model]]),
            paste0("Results/Profiles/withThyroid/",model,"_CL_RNA_norm.csv"))
