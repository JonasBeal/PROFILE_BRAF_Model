...
CL_clin <- read_csv("Data/CL/Infos/model_list_20200115.csv") %>%
  ...
  mutate(TCGA_label=if_else(cancer_type=="Colorectal Carcinoma", "CR_carcinoma",
                            if_else(cancer_type=="Melanoma", "SK_melanoma", cancer_type)))
cosmic_corres <- select(CL_clin,model_name,COSMIC_ID)

#Focus on two cancer types only
good_labels <- c("CR_carcinoma", "SK_melanoma")
...
polyphen <- read_delim("Data/Common/pph2-full.txt", delim = '\t', trim_ws = T) 
...
for (model in models){
  write.csv(inverse_mapping(CLmodel_mut_prof,
            ...
            paste0("Results/Profiles/",model,"_CL_mut.csv"))
  
  write.csv(inverse_mapping(CLmodel_CNA_prof,
                            datatype = 'bin',
                            geneindex_var = node_genes_correspondance[[model]]),
            paste0("Results/Profiles/",model,"_CL_CNA.csv"))

  write.csv(inverse_mapping(CLmodel_mutCNA_prof,
                            datatype = 'bin',
                            geneindex_var = node_genes_correspondance[[model]]),
            paste0("Results/Profiles/",model,"_CL_mutCNA.csv"))
  
  write.csv(inverse_mapping(CLmodel_mutCNAneg_prof,
                            datatype = 'bin',
                            geneindex_var = node_genes_correspondance[[model]]),
            paste0("Results/Profiles/",model,"_CL_mutCNAneg.csv"))
  
  write.csv(inverse_mapping(CLmodel_RNA_prof,
                            datatype = 'bin',
                            geneindex_var = node_genes_correspondance[[model]]),
            paste0("Results/Profiles/",model,"_CL_RNA.csv"))
  
  write.csv(inverse_mapping(CLmodel_RNA_prof_norm,
                            datatype = 'norm',
                            geneindex_var = node_genes_correspondance[[model]]),
            paste0("Results/Profiles/",model,"_CL_RNA_norm.csv"))
