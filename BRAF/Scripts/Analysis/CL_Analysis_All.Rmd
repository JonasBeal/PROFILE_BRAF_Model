...
list.of.packages <- c("readxl","knitr","corrplot","ggfortify","reshape2","gridExtra","magrittr", "tictoc", "ggrepel", "broom", "nlme", "lme4", "lmerTest", "eulerr", "plotly", "paletteer", "ggpubr", "cowplot", "ggcorrplot", "ggnomics", "tidyverse", "UpSetR", "latex2exp")
...
```{r plot_correlation_metrics, echo=FALSE, message=FALSE, warning=FALSE, eval=TRUE}
...
```{r correlations, echo=FALSE, fig.height=6, fig.width=9, message=FALSE, warning=FALSE}
...
```{r correlations_pub, echo=FALSE, fig.height=6, fig.width=9, message=FALSE, warning=FALSE, eval=TRUE}
...
plot_data <- filter(data_all_clin,
  ...
  plot_data2 <- filter(plot_data, Screening=="CRISPR") %>%
    mutate(Variant="Broad data", Label_Void=" ",
           Label_Select = if_else(Patient_ID %in% c("SK-MEL-24", "COLO-678", "HT-29", "HT-144", "HT55", "SK-MEL-30", "SK-MEL-2"),
                                  Patient_ID, " "))
    ...
    geom_label_repel(aes(label=Label_Void, fill=Label),
                     ...
                     box.padding = 0, label.padding = 0.02,
                     segment.color = NA) +
    geom_text_repel(data=subset(plot_data2, Patient_ID %in% c("SK-MEL-24", "HT-29","SK-MEL-30")),
...
```{r scatter_pub_table, echo=FALSE, message=FALSE, warning=FALSE, eval=TRUE, fig.height=6, fig.width=9}
...
plot_data_tab <- filter(data_all_clin,
  ...
  plot_data_mut <- left_join(plot_data_basis,
    ...
    mutate(Type="Mutations profile (discrete)")
  ...
  plot_data_RNA <- left_join(plot_data_basis,
    ...
    mutate(Type="RNA profile (continuous)")
  ...
  plot_data_tab2 <- bind_rows(plot_data_mut, plot_data_RNA) %>%
    ...
    bind_rows(mutate(plot_data_basis, Value=0.5, `Model nodes`="",
                     Value_char=Patient_ID, Type="Cell line")) %>%
   ...
   p_table_cl <- ggplot(plot_data_tab2,
     ...
     geom_text(aes(label=Value_char), size=2) +
...
```{r scatter_pub_add, echo=FALSE, message=FALSE, warning=FALSE, eval=FALSE, fig.height=6, fig.width=9}
